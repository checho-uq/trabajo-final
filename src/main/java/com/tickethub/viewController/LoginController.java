...
public class LoginController {
    ...
    private void handleLogin(ActionEvent event) {
        ...
        if (u != null) {
            ...
            try {
                String vista = u.isEsAdmin() ? "/com/tickethub/views/AdminDashboard.fxml" : "/com/tickethub/views/UserDashboard.fxml";
                Parent root = FXMLLoader.load(getClass().getResource(vista));
                Stage window = (Stage) txtCorreo.getScene().getWindow();
                window.setScene(new Scene(root, window.getWidth(), window.getHeight()));
            } catch (Exception e) { e.printStackTrace(); lblError.setText("Error al cargar: " + e.getMessage()); }
        } else {
    ...
    private void handleRegistro(ActionEvent event) {
        try {
            Parent root = FXMLLoader.load(getClass().getResource("/com/tickethub/views/Registro.fxml"));
